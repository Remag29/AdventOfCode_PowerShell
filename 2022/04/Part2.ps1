...
function Test-Overlaps {
    ...
    if (($elfe1.min -le $elfe2.min) -and ($elfe1.max -ge $elfe2.min)) {
        # elfe1 is in elfe2
        return $true
    }
    elseif (($elfe1.min -le $elfe2.max) -and ($elfe1.max -ge $elfe2.max)) {
        # elfe2 is in elfe1
        return $true
    }
    else {
...
$lines | ForEach-Object {
    ...
    if ((Test-RangeInOther -ElfesObject $elfesMinMax) -or (Test-Overlaps -ElfesObject $elfesMinMax)) {
        $finalResult++
    }
}
