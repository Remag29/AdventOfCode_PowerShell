...
function Invoke-StartRounds {
    ...
    $superModulo = 1
    $MonkeysList | ForEach-Object {
        $superModulo *= $_.DropTest
    }
    ...
    for ($round = 1; $round -le $TotalRound; $round++) {
        ...
        foreach ($monkey in $MonkeysList) {
            ...
            foreach ($item in $monkey.Item) {
                ...
                $item.WorryLevel = [System.Math]::Floor($item.WorryLevel % $superModulo)
...
Invoke-StartRounds -MonkeysList $monkeyList -TotalRound $round
