...
function Get-AllAPoints {
    param (
        [array] $Lines
    )
    
    $points = New-Object System.Collections.Generic.List[psobject]
    foreach ($line in $Lines) {
        for ($letterIndex = 0; $letterIndex -lt $line.Length; $letterIndex++) {
            if ($line[$letterIndex] -eq "a") {
                $point = New-Points -X $Lines.IndexOf($line) -Y $letterIndex -letter $line[$letterIndex] -Count 0 -heuristic 0
                $points.Add($point)
            }
        }
    }
...
function Invoke-Part2 {
    ...
    $aPoints = Get-AllAPoints -Lines $Lines
    ...
    foreach ($aPoint in $aPoints) {
        Update-Heuristic -Point $aPoint -Goal $EndPoint
        $openLists.Add($aPoint) | Out-Null
    }
...
$time = Measure-Command -Expression {
    $result = Invoke-Part2 -Lines $lines
