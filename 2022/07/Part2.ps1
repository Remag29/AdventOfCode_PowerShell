...
function Get-FileToDeleteScore {
    param (
        [Parameter(Mandatory = $true, Position = 0)] [psobject] $RootObject,
        [Parameter(Mandatory = $true, Position = 1)] [int32] $SizeToDelete,
        [Parameter(Mandatory = $false, Position = 2)] [int32] $BestScore
    )

    $RootObject.Children | ForEach-Object {
        if ($_.Type -eq "dir") {
            if ($_.Size -lt $bestScore -and $_.Size -ge $SizeToDelete) {
                $bestScore = $_.Size

                $bestChildScore = Get-FileToDeleteScore -RootObject $_ -SizeToDelete $SizeToDelete -BestScore $bestScore
                if ($bestChildScore -lt $bestScore) {
                    $bestScore = $bestChildScore
                }
            }
            else {
                $bestScore = Get-FileToDeleteScore -RootObject $_ -SizeToDelete $SizeToDelete -BestScore $bestScore
            }
        }
    }
    return $bestScore
...
$totalSpace = 70000000
$usedSpace = $rootObject.Size
$needToDelete = 30000000 - ($totalSpace - $usedSpace)
Write-Host "Need to delete $needToDelete" -ForegroundColor Red

$bestScore = Get-FileToDeleteScore -RootObject $rootObject -SizeToDelete $needToDelete -BestScore $totalSpace
Write-Host "File to delete: $bestScore" -ForegroundColor Cyan